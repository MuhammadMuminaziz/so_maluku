...
class Surat_Undangan_model extends CI_Model
{

	private $_table = 'surat_undangan';
	...
	public function get_by_current_user()
	{
		...
		$this->db->where('surat_undangan.status', UNDANGAN_APPROVED);
	...
	public function find_by_status($status)
	{
		...
		if ($level_jabatan == LEVEL_KABIRO && $id_jabatan == JABATAN_BIRO_UMUM) {
			...
			$where['surat_undangan.id_user'] = $current_user->id;
		}

		if ($status) {
			$where['surat_undangan.status'] = $status;
		}
		if ($or_status) {
			$or_where['surat_undangan.status'] = $or_status;
		}

		$this->db->select('surat_undangan.id, nomor_surat, lampiran, perihal, tanggal_surat, lokasi_kegiatan, surat_undangan.isi_pendahuluan, surat_undangan.isi_penutup, list_tembusan, surat_undangan.status, approved_by, surat_undangan.id_user id_pemohon, surat_undangan.doc, opd_from.nama_opd nama_opd_from, GROUP_CONCAT(opd_to.nama_opd SEPARATOR ", ") as nama_opd_to');
		$this->db->join('user', 'user.id = surat_undangan.id_user', 'left');
		$this->db->join('opd opd_from', 'opd_from.id = user.id_opd', 'left');
		$this->db->join('surat_undangan__opd', 'surat_undangan__opd.id_surat_undangan = surat_undangan.id', 'left');
		$this->db->join('opd opd_to', 'opd_to.id = surat_undangan__opd.id_opd', 'left');
		$this->db->where($where);
		$this->db->or_where($or_where);

		$this->db->group_by('surat_undangan.id');
		$query = $this->db->get($this->_table);
		return $query->result();
	}
	...
	public function find($id)
	{
		...
		$surat_undangan->list_id_opd = $this->db->get_where('surat_undangan__opd', array('id_surat_undangan' => $id))->result();
